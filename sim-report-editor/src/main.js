...
const state = {
  activeSlideId: 1,
  reportId: localStorage.getItem('sim-report-id') || 'report-2026-01',
...
function updateField(slideId, fieldId, value, source = 'editor') {
  ...
  localStorage.setItem(storageKey(slideId), JSON.stringify(state.data[slideId]));
}
...
function bindEvents() {
  ...
  document.querySelector('#report-id').addEventListener('change', event => {
    ...
    localStorage.setItem('sim-report-id', state.reportId);
